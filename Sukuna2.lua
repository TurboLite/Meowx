...
-- Biến điều khiển khả năng chạy nhanh
...
local function InfAb()
    ...
    if humanoid then
        if InfAbility then
            humanoid.WalkSpeed = 100 -- Tốc độ chạy nhanh (mặc định là 16)
        else
            humanoid.WalkSpeed = 100 -- Tốc độ chạy bình thường
        end
    end
end

-- Vòng lặp để duy trì khả năng chạy nhanh
spawn(function()
    while wait() do
        if InfAbility then
            InfAb()
