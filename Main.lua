...
-- Hàm tìm vũ khí trong nhân vật hoặc backpack
local function getTool()
    return character:FindFirstChildWhichIsA("Tool") or player.Backpack:FindFirstChildWhichIsA("Tool")
end

-- Tấn công nhanh
while true do
    wait(0.1) -- Điều chỉnh chu kỳ lặp (0.1 giây là đủ để tránh lag)
    
    if humanoid.Health > 0 then
        local tool = getTool()
        
        if tool then
            -- Chỉ trang bị vũ khí nếu chưa trang bị
            if not humanoid:FindFirstChildOfClass("Tool") then
                humanoid:EquipTool(tool) -- Trang bị vũ khí
            end
            
            -- Kích hoạt công cụ
            pcall(function()
                if tool:FindFirstChild("Activate") then
                    tool:Activate() -- Kích hoạt đòn tấn công nếu công cụ hỗ trợ
                end
            end)
        end
    else
        break -- Dừng vòng lặp nếu nhân vật chết
    end
end
...
local Toggle = Tabs.item:AddToggle("Start Farm", { Title = "Bắt Đầu Farm", Description = "", Default = false })
Toggle:OnChanged(function(Value)
    StartFarm = Value
    StopTween(StartFarm)
end)
